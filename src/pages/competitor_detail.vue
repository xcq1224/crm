    ...
    <div class="page"> 
        <swiper class="crm_detial_header" dots-class="dots" :auto='false' dots-position="center">
            <swiper-item class="item1">
                ...
                <p>竞争力：{{baseInfo.power || '———'}}</p>
            </swiper-item>
            <swiper-item class="item2">
                <p>参与成员：</p>
                <router-link :to="'./client_member?id=' + baseInfo.id + '&moduleName=crm_competitor&moduleNames=竞争对手&isPublic=' + baseInfo.isPublic" class="user-list">
        ...
        <div class="main">
            <tab class="vux-1px-b" :line-width=2 custom-bar-width="80%" :scroll-threshold='7' v-model="tabIndex">
                ...
                <tab-item class="vux-center" :selected="tabIndex == 1" @click="tabIndex = 1">机会</tab-item>
                <tab-item class="vux-center" :selected="tabIndex == 2" @click="tabIndex = 2">附件</tab-item>
                <tab-item class="vux-center" :selected="tabIndex == 3" @click="tabIndex = 3">日志</tab-item>
            </tab>
            <swiper v-model="tabIndex" :show-dots="false" height="100%" @on-index-change="getInfoByindex">
                <swiper-item class="tab-item2">
                    ...
                    <div class="footer" v-if="isModify">
                        <router-link class="footer-tab" :to="'./competitor_add?handleType=1&id=' + baseInfo.id">编辑</router-link>
                        <div v-if="isOwner" class="footer-tab vux-1px-l" @click="popup2 = true">转给他人</div>
                        <div v-if="isOwner" class="footer-tab vux-1px-l" @click="del">删除</div>
                    </div>
                </swiper-item>
                <swiper-item class="tab-item4">
                    <div class="scroll-box" style="padding-bottom: 0;">
...
<script>
    ...
    export default {
        ...
        methods: {
            //  获取团队成员
            getMembers(){
                this.$post("/crm/memberPR/getAll", {"parentId": this.query.id, "moduleName": "crm_competitor"}, (data) => {
            ...
            del(){
                this.$vux.confirm.show({
                    ...
                    onCancel: () => {},
                    onConfirm: () => {
                        this.$post('/crm/competitorPR/delete', {"list": [this.baseInfo.id]}, (data) => {
                            if(data.message){
        ...
        /**---------------------------------------------转给他人--------------------------------------------- */
            ...
            turnSave(){
                ...
                this.$post("/crm/competitorPR/turnOther", params, (data) => {
                    this.popup2 = false
